...
async function cargarDatos() {
    try {
        ...
        modeloBadge.innerHTML = '<i class="bi bi-exclamation-triangle me-1"></i> Error al cargar datos';
...
function buscarRecursivo(menu, query, rutaActual = '', nivel = 0) {
    ...
    if (typeof menu === 'object' && menu !== null && !Array.isArray(menu)) {
        ...
        if (nombreActual.toLowerCase().includes(query.toLowerCase())) {
            const posicionTexto = posicionActual ? `#${posicionActual} en la lista` : 'Elemento principal';
            resultados.push({
                nombre: nombreActual,
...
function mostrarResultados(resultados, query) {
    ...
    // Mostrar resultados
    resultadosDiv.innerHTML = resultados.map((r, index) => {
        let icon = '📌';
        if (r.nivel === 0) icon = '📂';
        else if (r.nivel === 1) icon = '📁';
        else if (r.nivel >= 2) icon = '📄';
        
        const nivelText = r.nivel === 0 ? 'Menú' : 
                         r.nivel === 1 ? 'Submenú' : 
                         `Nivel ${r.nivel}`;
...
buscador.addEventListener('input', function() {
    ...
    if (query.length === 0) {
        searchHint.innerHTML = '<i class="bi bi-lightbulb me-1"></i> Escribe al menos 2 caracteres para buscar';
        mostrarEstadoVacio();
        estadisticasDiv.style.display = 'none';
        return;
    }
    ...
    if (query.length < 2) {
        searchHint.innerHTML = '<i class="bi bi-keyboard me-1"></i> Escribe al menos 2 caracteres...';
        resultadosDiv.innerHTML = `
            <div class="text-center py-5">
                <div class="display-1 mb-3">⌨️</div>
